...
$allowedTopLevel = @(
    '.gitattributes', '.github', '.githooks', '.gitignore', 'AGENTS.md',
    'assets', 'CONTRIBUTING.md', 'docs', 'examples', 'LICENSE',
    'PUBLIC_BOUNDARY.md', 'README.md', 'requirements.md', 'scripts',
    'SECURITY.md', 'src', 'tests'
)

$blockedTopLevel = @(
    '.openai', '.wrangler', 'app', 'audit-output', 'branding', 'build',
    'collected-scripts', 'db', 'dist', 'drizzle', 'lib', 'marketing',
    'node_modules', 'public', 'worker'
)

$blockedFileNames = @(
    '.dev.vars', '.env', '.env.local', '.npmrc', '.pypirc',
    'credentials.json', 'id_ed25519', 'id_rsa', 'package-lock.json',
...
try {
    ...
    foreach ($relativePath in $relativePaths) {
        ...
        if (($blockedTopLevel -contains $topLevel) -or
            ($topLevel -like 'At0mFlow-RepoSync-*')) {
    ...
    if (Test-Path -LiteralPath (Join-Path $repositoryRoot '.git')) {
        ...
        if ($remotes -contains 'origin') {
            ...
            $validRemote = $origin -match '^(?:https://github\.com/At0mFlow/At0mFlow-RepoSync(?:\.git)?|git@github\.com:At0mFlow/At0mFlow-RepoSync\.git)$'
            if (-not $validRemote) {
